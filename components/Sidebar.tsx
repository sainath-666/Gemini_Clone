...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle, sessions, activeSessionId, onNewChat, onSelectChat }) => {
  return (
    <>
      <div className={`fixed top-4 left-4 z-20 md:hidden ${!isOpen ? 'block' : 'hidden'}`}>
        <button onClick={onToggle} className="p-2 text-neutral-300 hover:text-white">
      ...
      >
        <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={onToggle} className="p-2 md:hidden text-neutral-300 hover:text-white">
                <MenuIcon className="w-6 h-6" />
            </button>
        </div>
        ...
        <div className="mt-8 flex-1 overflow-y-auto">
          <p className="text-sm font-medium text-neutral-400 px-3 mb-2">Recent</p>

...
interface ChatViewProps {
  ...
  onSendMessage: (prompt: string, isNewChat: boolean) => void;
  isLoading: boolean;
}

const ChatView: React.FC<ChatViewProps> = ({ activeSession, onSendMessage, isLoading }) => {
  ...
  const handleSendPrompt = (prompt: string) => {
    onSendMessage(prompt, !activeSession || activeSession.messages.length === 0);
  };
    ...
    <div className="flex flex-col h-full flex-1 bg-[#131314]">
      <div className="flex-1 overflow-y-auto">
        {hasMessages ? (
          ...
          <WelcomeView onPromptClick={handleSendPrompt} />
        )}
      </div>
